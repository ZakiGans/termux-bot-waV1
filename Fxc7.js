...
// Author I AM GANS 
// LARANG AMBIL CASE GUA COK
// REMAKE SC INI?? TAMBAHIN NAMA GUA ZAKIGANS
// AWAS AJA LU KETAHUAN REMAKE GK ADA NAMA GUA
// Tambahin ya Cape Gan ngefix² Yg Ga work
// Jan numpang nama doank
// MODAL NUMPANG NAMA ANJENG 
...
prefix = "!"
name = "~ X BOT"
rdaftar = "TERIMA KASIH TELAH DAFTAR MENJADI TEMEN XBOT😁"
rmenu = "HAI TEMEN XBOT👋 JANGAN LUPA DONASI YAA:)"
...
async function starts() {
	...
	frhan.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Sayonara @${num.split('@')[0]} JASA MU SANGAT MEMBANGGONG KAN`
				let buff = await getBuffer(ppimg)
				frhan.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	frhan.on('chat-update', async (mek) => {
       ...
       if (messagesLink.includes("://chat.whatsapp.com/")){
		...
		case 'daftar':
					frhan.updatePresence(from, Presence.composing)
					if (isUser) return reply('kamu sudah Menjadi Temen X BOT:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			if (!isPublic) return reply(mess.only.publikG)
                ...
                case 'xbotgroup':
                   ...
                   client.sendMessage(from, rules(prefix), text, { quoted: mek })
                    ...
                    tod = await getBuffer(`https://i.ibb.co/PcQ6tsB/79ac87b9358c.jpg`)
	...
	.then(() => {client.sendMessage(from, options, text)})
         ...
         case 'lirik':
                ...
                if (!isPublic) return reply(mess.only.publikG)
                    ...
                    frhan.sendMessage('6283144780782@s.whatsapp.net', options, text, {quoted: mek})
              ...
              if (args[1]=="detik") {var timer = args[0]+"000"
                ...
                if (!isOwner) return reply('Kamu siapa?')
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *SANG LORD BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
			...
			if (isBanned) return reply(mess.only.benned)    
               ...
               if (anu.error) return reply(anu.error)
                ...
                case 'indo1':
					if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				if (!isPrem) return reply('Anda bukan Member Premium, silakan hubungi owner atau ketik *#daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
